...
use EQingdan\SensorsAnalytics\Exceptions\DebugException;
...
class DebugConsumer extends AbstractConsumer
{
    ...
    public function __construct($url_prefix, $write_data = true, $request_timeout = 1000)
    {
        ...
        if ($parsed_url === false) {
            throw new DebugException("Invalid server url of Sensors Analytics.");
        }

        // 将 URI Path 替换成 Debug 模式的 '/debug'
        $parsed_url['path'] = '/debug';

        $this->_debug_url_prefix = ((isset($parsed_url['scheme'])) ? $parsed_url['scheme'] . '://' : '') . ((isset($parsed_url['user'])) ? $parsed_url['user'] . ((isset($parsed_url['pass'])) ? ':' . $parsed_url['pass'] : '') . '@' : '') . ((isset($parsed_url['host'])) ? $parsed_url['host'] : '') . ((isset($parsed_url['port'])) ? ':' . $parsed_url['port'] : '') . ((isset($parsed_url['path'])) ? $parsed_url['path'] : '') . ((isset($parsed_url['query'])) ? '?' . $parsed_url['query'] : '') . ((isset($parsed_url['fragment'])) ? '#' . $parsed_url['fragment'] : '');
    ...
    public function send($msg)
    {
        $buffers = array();
        $buffers[] = $msg;
        $response = $this->doRequest(array(
            "data_list" => $this->encodeMsgList($buffers),
            "gzip" => 1,
        ));
        printf("\n=========================================================================\n");
        ...
        if ($response['ret_code'] >= 300) {
            throw new DebugException("Unexpected response from SensorsAnalytics.");
        }
    }
    ...
    protected function doRequest($data)
    {
        ...
        if (!$http_response_header) {
            throw new DebugException("Failed to connect to SensorsAnalytics. [error='" . curl_error($ch) . "']");
        }
